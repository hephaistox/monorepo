(ns tasks.local
  ...
  (:require [auto-build.project.cfg-mgt :refer
             [project-dir is-git-repo? extract-project]]
          ...
          (let [[app-fullname & rapp-fullnames-to-check] app-fullnames-to-check
            ...
            (cond (seq dep-to-check)
                    (let [{:keys [path dep-alias dep-desc deps-dir]}
                            dep-to-check
                          {:local/keys [root]} dep-desc
                          app-dir (-> dep-alias
                                      str
                                      extract-project
                                      project-dir)]
                      (if root
                        (normalln (uri-str path) "targets local already")
                        (do (update-deps-edn printers
                                             deps-dir
                                             (concat path [dep-alias])
                                             {:local/root (str "../" app-dir)})
